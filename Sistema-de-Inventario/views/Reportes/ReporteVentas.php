...
class PDF extends FPDF
{
    ...
    // Cabecera de página
      function Header()
      {
         ...
         $this->Cell(25, 8, utf8_decode('ID'), 1, 0, 'C', 1);
         $this->Cell(50, 8, utf8_decode('FECHA DE VENTA'), 1, 0, 'C', 1);
         $this->Cell(65, 8, utf8_decode('CLIENTE'), 1, 0, 'C', 1);
         $this->Cell(40, 8, utf8_decode('TOTAL'), 1, 0, 'C', 1);
         $this->Ln(8);
      }
...
$sqlVentas = $conexion->query("
   SELECT 
      ...
      DATE_FORMAT(ventas.FechaVenta, '%d-%m-%Y') as FechaVenta,
      TotalVenta
   FROM 
      ventas
   JOIN 
...
while ($row = $sqlVentas->fetch_object()) {
      $pdf->SetFont('Arial', 'B', 10);      
      $pdf->Cell(25, 8, utf8_decode($row->idVenta), 1, 0, 'C', 0);
      $pdf->Cell(50, 8, utf8_decode($row->FechaVenta), 1, 0, 'C', 0);
      $pdf->Cell(65, 8, utf8_decode($row->NombreCliente), 1, 0, 'C', 0);
      $pdf->Cell(40, 8, utf8_decode('$'.$row->TotalVenta), 1, 0, 'C', 0);
      $pdf->Ln(8);
      ...
      $sqlDetalleCompra = $conexion->query("CALL obtenerDetalleVentaFiltro(".$row->idVenta.")");
