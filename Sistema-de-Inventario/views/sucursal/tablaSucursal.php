<?php
    ...
    // Verificar si se recibieron datos del formulario al agregar un Sucursal
    if ($_POST && isset($_POST['TXTnombreS'], $_POST['TXTubicacion'], $_POST['TXTdepartamento'], $_POST['TXTmunicipio'])) {
        ...
        if ($nombreS != "" && $ubicacion != "" && $departamento != "" && $municipio != "") {
            // Muestra un mensaje de éxito al agregar un Sucursal y redirige a la tabla de Sucursal (caso en php)
            echo "<script>
                window.onload = function() {
                    Swal.fire({
                        title: '¡Éxito!',
                        text: 'Sucursal agregado exitosamente',
                        icon: 'success'
                    }).then((result) => {
            ...
            $data = $objSucursal->crearSucursal($nombreS, $ubicacion, $departamento, $municipio);
        }else{
            // Muestra un mensaje de error al agregar un Sucursal y redirige a la tabla de Sucursal (caso en php)
            echo "<script>
                window.onload = function() {
                    Swal.fire({
    ...
    // Verificar si se ha solicitado la eliminación de un Sucursal
    if (isset($_POST['delete_id'])) {
        $idSucursal = $_POST['delete_id'];
        echo "<script>
        window.onload = function() {
            Swal.fire({
                title: '¡Éxito!',
                text: 'Sucursal eliminado exitosamente',
                icon: 'success'
            }).then((result) => {
                if (result.isConfirmed) {
                    window.location.href = './tablaSucursal.php';
        ...
        $objSucursal->eliminarSucursal($idSucursal); 
    }
    ?>
...
<body class="bg-light">
                    ...
                    <ul class="navbar-nav justify-content-end flex-grow-1 pe-3">
                        ...
                        <li class="nav-item dropdown">
                            ...
                            <li><a class="dropdown-item" id="Usuarios" href="../../views/usuarios/tablaUsuario.php">Usuarios</a></li>
                                    ...
                                    <li><a class="dropdown-item" id="Sucursales" href="../sucursal/tablaSucursal.php">Sucursales</a></li>
    ...
    <div class="container mt-4 bg-white rounded p-4 shadow">
        <div class="row">
            ...
            <div class="col-md-4 text-lg-center">
                ...
                <a class="btn btn-success btn-lg" href="../../views/Reportes/ReporteSucursales.php" target="blank">Generar Reporte</a>
            </div>
        </div>
        <hr>
        <div class="table-responsive mt-2">
            <table class="table table-bordered table-hover" id="tabla-datos">
                ...
                <tbody>
                    ...
                    <?php
                    // Obtener todos los datos de las Sucursal
                    $data=$objSucursal->obtenerSucursales();
                    // Recorrer cada dato que existe y mostrarlos en la tabla
                    foreach( $data as $objSucursal){
                        echo"<tr>";
                        echo "<td>".$objSucursal["idSucursal"]."</td>";
                        echo "<td>".$objSucursal["NombreSucursal"]."</td>";
                        echo "<td>".$objSucursal["Ubicacion"]."</td>";
                        echo "<td>".$objSucursal["Departamento"]."</td>";
                        echo "<td>".$objSucursal["Municipio"]."</td>";
                        $id = $objSucursal["idSucursal"];
                        echo "<td data-bs-toggle='modal' data-bs-target='#modal-editar' class='action-buttons'>
                            <button class='btn btn-warning btn-lg btn-spacing editar-btn'><a class='text-decoration-none text-dark' href='./viewModificarSucursal.php?id=$id'>Editar</a>
                            </button><button class='btn btn-danger btn-lg btn-spacing eliminar-btn' data-id='".$objSucursal["idSucursal"]."'>Eliminar</button>
                            </td>";
                            echo"</tr>";
                    }
    ...
    <!-- Modal para agregar un Sucursal -->
    <div class="modal fade" id="modal-agregar" tabindex="-1" aria-labelledby="modal-agregar-label" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modal-agregar-label">Formulario de registro de la Sucursal</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form action="./tablaSucursal.php" method="POST" autocomplete="off">
                        <div class="row">
                            <div class="form-group col-md-6">
                                <input type="text" class="form-control" name="TXTnombreS" id="Nombre" placeholder="Nombre" require>
                            </div><br><br>
                            <div class="form-group col-md-6">
                                <input type="text" class="form-control" name="TXTubicacion" id="Ubicacion" placeholder="Ubicacion" require>
                            </div><br><br>
                            <div class="form-group col-md-6">
                            <select class="form-select" name="TXTdepartamento" id="Departamento" placeholder="Departamento" require>
                                    <option selcted disable>-- Seleccione el departamento --</option>
                            ...
                            <div class="form-group col-md-6">
                                <input type="text" class="form-control" name="TXTmunicipio" id="Municipio" placeholder="Municipio" require>
                            </div><br><br>
                        </div>
    ...
    <!-- Formulario oculto para eliminar Sucursal -->
    <form id="delete-form" action="./tablaSucursal.php" method="POST" style="display: none;">
